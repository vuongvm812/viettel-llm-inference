...
| # | Command | Catches |
|---|---|---|
| 1 | `make vllm-fork PUSH=1` — **CLOSED again 2026-07-27, no rebuild owed** | patch/version drift; the `patch --dry-run` gate. The `c5bea8bf` digest this row used to warn about is gone: `VLLM_FORK_DIGEST` was re-pinned in `c1b408f`, which post-dates `e141e9c` (the last commit to touch any `.patch`). The 2026-07-27 reconciliation changed **no** patch — `gen.sh` regenerates all eight byte-identically — so the fork image content is unchanged and the pin stands. Rebuild only when a `.patch` actually moves. Runtime confirmation is free: `v2_input_buffers.patch` and `v2_prepare_inputs.patch` are a pair, and an image missing either raises `AttributeError` on the very first decode step, so a server that streams one token has both. |
