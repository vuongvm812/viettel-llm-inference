...
The reorder runs on EVERY ``schedule()`` call and carries ONE guard: the queue is shorter
than 2. That is the common case here -- mean concurrency is 2, so ``waiting`` is nearly
always empty or single-entry and we return before touching the cache.

REVERTED 2026-07-26: a caching layer (a queue fingerprint + an N-step re-sort TTL, plus an
"admission is capped anyway" short-circuit) used to sit here to skip redundant sorts. It was
removed after the commit that added it measured SLOWER. It could not have paid for itself:
at this concurrency the ``len < 2`` return above fires first on nearly every step, so the
guards only ever added per-step work to the path they were meant to shorten, and their one
real effect was to let a stale admission order persist for up to N steps. Do not re-add
caching here without a queue deep enough to sort in the first place.
