...
This is the regression guard for "L2 128B alignment of KV blocks", which the round-1.2
...
whole file GPU-box-only). References into the served v0.25.0 tree:
  ...
  vllm/model_executor/layers/mamba/mamba_utils.py  short_conv_state_shape
...
without it (so it is real only inside the container, via ``make test-kernel``).
    ...
    python3 round-1.2/bench/test_kv_alignment.py
"""

from math import prod

# --- served LFM2.5-1.2B geometry (hf-model/config.json + docker-compose.yaml) -------------
BLOCK_SIZE = 16  # vLLM default; compose does not pass --block-size
NUM_KV_HEADS = 8  # num_key_value_heads
HEAD_SIZE = 64  # hidden_size 2048 / num_attention_heads 32
KV_DTYPE_BYTES = 1  # --kv-cache-dtype=fp8_e4m3
NUM_ATTN_LAYERS = 6  # layer_types: 6 x full_attention
CONV_DIM = 2048  # conv_dim
CONV_L_CACHE = 3  # conv_L_cache
CONV_DTYPE_BYTES = 2  # bf16 conv state (mamba_cache_dtype defaults to model dtype)
NUM_CONV_LAYERS = 10  # layer_types: 10 x conv
...
def conv_page_size_bytes(conv_dim, conv_l_cache, dtype_bytes, num_spec=0):
    """``MambaSpec.page_size_bytes`` for LFM2's single short-conv state tensor."""
    shapes = (short_conv_state_shape(conv_dim, conv_l_cache, num_spec),)
    return sum(prod(shape) * dtype_bytes for shape in shapes)


def short_conv_state_shape(conv_dim, conv_l_cache, num_spec=0):
    return (conv_dim, conv_l_cache - 1 + num_spec)


def test_lfm2_page_sizes_are_l2_aligned():
    ...
    for name, page in (("attention", attn), ("short-conv", conv)):
...
def test_kv_buffer_base_and_block_offsets_are_aligned():
    ...
    for name, page, num_layers in (
        ...
        ("short-conv", conv_page_size_bytes(CONV_DIM, CONV_L_CACHE, CONV_DTYPE_BYTES), NUM_CONV_LAYERS),
    ):
...
def main() -> None:
    test_lfm2_page_sizes_are_l2_aligned()
