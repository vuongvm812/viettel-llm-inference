...
// emits for the [2048]-wide `operator_norm -> qkv/in_proj` and `ffn_norm -> w13` pairs -- on
// every pre-norm block of the served model (a 2048-wide hidden is the worked example; the
// [64] F32 per-head QK-norms are a different op and not fused here). Stock does three passes over input+residual
...
// port of stock. Not reached at the common bf16 hidden sizes, which take the fast path above.
template <typename scalar_t, bool kHasResidual>
