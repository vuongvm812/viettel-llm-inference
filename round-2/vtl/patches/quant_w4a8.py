...
step a ~1B-parameter model streams its whole body + a bf16 lm_head:
...
# or a per-SHAPE map, since a model's distinct GEMM shapes want different tiles and a boot is
...
# A schedule key that matches no layer fails the worst way possible: the boot succeeds, the arm
# is never selected, and the sweep row reads as a duplicate baseline. So keys are checked -- but
# against what the LOADED model actually has, not a baked-in list, because the shapes are a
# property of the model mounted at /model and nothing else.
#
# Two halves, and neither can be a hard rejection (a TP split legitimately changes every number):
#   * VTL_W4A8_EXPECT_SHAPES -- optional comma-separated allowlist ("n3072k2048,n2048k8192").
#     Unset (the default) = no key is warned about, because we have nothing to compare against.
#   * _observed_shape_keys   -- filled in as layers are quantized and printed by
#     log_fallback_summary(). THIS is how you discover the values to put in the env var above:
#     boot once, read the summary line, paste the keys back.
_observed_shape_keys: set[str] = set()


def expected_shape_keys(raw: str | None = None) -> frozenset[str]:
    """Allowlist from ``VTL_W4A8_EXPECT_SHAPES``. Empty = unknown model, so check nothing."""
    if raw is None:
        raw = os.environ.get("VTL_W4A8_EXPECT_SHAPES", "")
...
def log_fallback_summary() -> None:
    ...
    if _observed_shape_keys:
        # The tile-schedule tuning surface for THIS model, discovered at load. Paste into
        # VTL_W4A8_SCHEDULE_V2 / VTL_W4A8_EXPECT_SHAPES.
        log.info(
            "vtl: w4a8 observed shape keys: %s", ",".join(sorted(_observed_shape_keys))
        )
...
def schedule() -> str | None:
    ...
    process_weights_after_loading, and any hand-written call site reads the layer attribute. Same trick as
...
def parse_schedule_v2(raw: str | None, valid: frozenset[str] = VALID_SCHEDULES_V2,
    ...
    for item in text.split(";"):
        ...
        expected = expected_shape_keys()
        if key != "*" and expected and key not in expected:
            log.warning(
                "vtl: %s key %r is not in VTL_W4A8_EXPECT_SHAPES, so it will never fire "
                "(expected shapes: %s)",
                env, key, ", ".join(sorted(expected)),
            )
        mapping[key] = value
    ...
    class VtlW4A8LinearMethod(LinearMethodBase):
        ...
        def process_weights_after_loading(self, layer) -> None:
            ...
            try:
                ...
                # NOTE for anyone adding a hand-written kernel: this is the LAST point at which
                # the bf16 weight is readable. `weight_packed`/`weight_group_scale` are CUTLASS's
                # internal mixed-input tile orders and cannot be reinterpreted as a plain int4
                # matrix, so a kernel wanting its own weight view must re-derive it here, from
                # this tensor, before the line below drops it.
                #
                ...
                # Discovery, not validation: whatever shapes this model actually presents.
                # log_fallback_summary() prints them so VTL_W4A8_SCHEDULE_V2 can be written
                # against real keys instead of guessed ones.
                _observed_shape_keys.add(f"n{chan_scales.shape[0]}k{in_features}")
        ...
        def apply(self, layer, x, bias=None):
            ...
            # NOTE: a hand-written fused path that calls ops.cutlass_w4a8_mm directly with an
            # already-quantized activation does NOT come through here. For every layer that does,
            # the quant below is what RMSNormQuantFusionPass hoists into the norm.
...
def _self_check() -> None:
    ...
    # The allowlist is opt-in: unknown model (unset) means we have nothing to compare a key
    # against, so nothing is warned about. Guessing here would warn on every key of every model
    # this workspace is not yet tuned for.
    assert expected_shape_keys("") == frozenset()
    assert expected_shape_keys(" n1k2 , n3k4 ,,") == frozenset({"n1k2", "n3k4"})
    ...
    # The "never fire" half only speaks when VTL_W4A8_EXPECT_SHAPES says what real looks like.
    seen = []
    handler = logging.Handler()
    handler.emit = seen.append
    log.addHandler(handler)
    os.environ["VTL_W4A8_EXPECT_SHAPES"] = "n3072k2048,n2048k8192"
    try:
        ...
        os.environ.pop("VTL_W4A8_EXPECT_SHAPES", None)
    ...
    # ...and with no allowlist, the same impossible shape is silent.
    quiet = []
    handler2 = logging.Handler()
    handler2.emit = quiet.append
    log.addHandler(handler2)
    try:
        assert parse_schedule_v2("n2048k3072=128x8_1x1x1") == {"n2048k3072": "128x8_1x1x1"}
    finally:
        log.removeHandler(handler2)
    assert not any("never fire" in r.getMessage() for r in quiet), [r.getMessage() for r in quiet]
