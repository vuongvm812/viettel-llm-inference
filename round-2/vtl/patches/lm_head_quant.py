"""Quantize ``lm_head`` -- typically the last large bf16 tensor in the decode path.
...
WHY THIS WORKS WITHOUT UNTYING THE EMBEDDINGS, when the model ties them. With
``tie_embedding: true`` the model calls ``lm_head.tie_weights(embed_tokens)``, which routes to
...
def _fp8_method_cls():
    ...
    for a hand-written fused path). No third spelling of fp8 in this repo.
    """
    import torch
    from vllm import _custom_ops as ops
