...
``_C::rms_norm_dynamic_per_token_quant`` node -- every RMSNorm whose output feeds an
fp8 linear (in a pre-norm transformer block: the attention norm -> qkv and the MLP norm
-> w13), all but the final ``embedding_norm``, which feeds the bf16 ``lm_head``. The
mechanism is model-agnostic: it overrides the op's CUDA kernel, not any per-layer
count. That fused op, not the bare RMSNorm, is what actually runs.
