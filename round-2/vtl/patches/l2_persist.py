...
2. **Window (``VTL_L2_PERSIST=1``, default off).** Only useful if the probe reports
   ...
   The usual candidates are the per-layer MLP ``w2`` (~8 MB packed) and ``w13``
   (~16 MB packed).
...
# enum cudaLimit
# cudaLimitPersistingL2CacheSize. NOT 0x05 -- that is cudaLimitMaxL2FetchGranularity
# (valid range 0-128 bytes), which rejects any MiB-scale value with invalid argument.
LIMIT_PERSISTING_L2_CACHE_SIZE = 0x06
...
def largest_packed_weight(model):
    ...
    (typically ``w13`` and ``w2`` dominate). One window per stream, so the union
