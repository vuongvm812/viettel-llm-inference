...
def rust_config(num_blocks=4096, block_size=16, max_model_len=32768, radix=False):
    """A hybrid shape: one full-attention group + one mamba/align group."""
    return {
...
def unitary_config(num_blocks=4096, block_size=16, max_model_len=32768, radix=False):
    """A plain dense model: ONE full-attention group, no recurrent state.

    This is what a non-hybrid model produces, and it is the shape this workspace is most
    likely to actually serve -- yet every other test here drives the hybrid fixture. The
    two exercise different code: `coordinator.rs` sets `is_hybrid = n > 1`, so a single
    group takes the unitary path and never runs the hybrid fixed point at all.
    """
    cfg = rust_config(num_blocks, block_size, max_model_len, radix)
    cfg["groups"] = [g for g in cfg["groups"] if g["kind"] == "full"]
    return cfg


@requires_crate
def test_unitary_single_group_prefix_hits_match_the_oracle():
    """Dense-model parity: with one group the hit is just the longest cached chain prefix.

    No mamba group means no fixed point and no `num_tokens - 1` clamp -- the hybrid
    oracle's two extra rules must NOT leak into this path.
    """
    block_size = 16
    none_hash = vtl_sched.none_hash_from_seed("0")
    kv = vtl_sched.KvManager(unitary_config(num_blocks=1 << 15))
    assert kv.num_groups == 1, kv.num_groups
    ...
    for c, t in order:
        ...
        # Oracle: walk the chain while each block hash is cached. vLLM still refuses to
        # hit the whole prompt (it must leave one token to actually run), hence max_blocks.
        max_blocks = (len(tokens) - 1) // block_size
        expect = 0
        while expect < max_blocks and hashes[expect] in cached:
            expect += 1
        ...
        for h in hashes[: target // block_size]:
            cached.add(h)

    assert cached, "the workload must actually populate the cache or this proves nothing"
...
def _packer():
    ...
    except Exception:  # pragma: no cover - PYTHONPATH without round-2
...
def test_update_step_pack_np_fold_cache_is_idempotent_with_an_explicit_call():
    ...
    none_hash = vtl_sched.none_hash_from_seed("0")
    kv = vtl_sched.KvManager(rust_config(num_blocks=256))
    kv.store_arm(none_hash)
    slot = kv.intern("r")
    kv.set_stop_params(slot, 0, 1000, 2, [])
    kv.store_init(slot, [], 16, 0)
    # The table below declares 16 computed tokens = 1 full block, so the slot must actually
    # HAVE that block, with a hash for it. cache_blocks (which the fold runs internally)
    # indexes req_blocks[i] and block_hashes[i] for every full block; on a slot that was
    # never allocated both are empty and the fold panics before the test reaches a single
    # assertion. Pushing hashes and allocating gives the fold something real to fold.
    tokens = list(range(block_size))
    kv.push_hashes(slot, b"".join(vtl_sched.block_hashes(none_hash, block_size, tokens)), len(tokens))
    assert kv.allocate_slots(slot, block_size, 0, True, 0, 0, len(tokens), 0, False)
