...
> **HISTORICAL (phase-1 archive).** This ledger is kept for provenance only. Its analysis targeted
> earlier stacks (dense `Qwen2ForCausalLM`, then the `Qwen3_5ForConditionalGeneration` VL hybrid).
> **The served model is now `Lfm2ForCausalLM` (LFM2.5-1.2B): 16 layers = 10 short-conv + 6 GQA
> attention, no GDN/gated-delta-net, no MTP, no vision, no MoE.** The GDN/FLA and MTP rows below are
> therefore moot again, and the vtl plugin dropped its GDN kernels + attn sigmoid-gate fusion. The
> live patch set is the model-agnostic fp8 / rms-norm-quant / silu-mul-quant + scheduler patches.
>
> --- original (superseded) note follows ---
>
> This ledger's premise — *stock vLLM v0.22.1 + a dense `Qwen2ForCausalLM`* — no longer holds. The
> served model is `Qwen3_5ForConditionalGeneration` (a qwen3_5 VL hybrid: linear-attn/GDN + full-attn
> + MTP + vision) on **vLLM v0.25.0**. So the "GDN/FLA", "MoE", and "MTP" families the table below
> mark *skipped as irrelevant* are now **in scope**, and `inputs_embeds_optional` was **removed**.
