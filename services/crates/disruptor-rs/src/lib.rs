...
mod consumer;
mod cursor;
/// Lossy, **non-gating** broadcast SPMC ring (single producer, N independent
/// consumers). Unlike the gated LMAX modes in this crate, the producer never
/// reads consumer cursors and never blocks: a consumer that falls behind is
...
pub use crate::builder::{ProcessorSettings, build_multi_producer, build_single_producer};
pub use crate::consumer::event_poller::{Branch, EventGuard, EventPoller, Polling};
...
mod tests {
    ...
    fn mpsc_full_ringbuffer() {
        ...
        let mut producer1 =
            build_multi_producer(64, factory(), Sleep::new(Duration::from_millis(1)))
                .handle_events_with(processor)
                .build();
        ...
        drop(producer2);
        let result: Vec<_> = r.iter().collect();
        assert_eq!(result, (0..=65).collect::<Vec<i64>>());
    }
    ...
    fn mpsc_insufficient_space_for_batch_publication() {
        ...
        let mut expected = (0..58).collect::<Vec<i64>>();
    ...
    fn spmc_with_event_pollers() {
...

        {
            ...
            // `ExactSizeIterator::len` is implemented for `&mut EventGuard`, so the
            // mutable borrow is required despite clippy's suggestion.
            #[allow(clippy::unnecessary_mut_passed)]
            let guard_len = (&mut guard_1).len();
            assert_eq!(2, guard_len);
            assert_eq!(ep2.poll().err(), Some(Polling::NoEvents));
    ...
    fn spsc_back_pressure_through_out_of_band_branch_poller() {
        ...
        assert_eq!(
            RingBufferFull,
            producer.try_publish(|e| e.num = 99).err().unwrap()
        );
        ...
        producer
            .try_publish(|e| e.num = 100)
            .expect("Should publish after all consumers advanced.");
