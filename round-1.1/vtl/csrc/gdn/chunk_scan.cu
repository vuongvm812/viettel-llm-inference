...
// shim (vtl/patches/gdn_prefill_backend.py) routes vLLM's chunk_gated_delta_rule prefill to this
// ONLY when the layout matches, else falls back to the stock FlashInfer/Triton/CuteDSL kernel.
//
// SCOPE / honest ceiling (ponytail: sequential baseline, WY-parallel body is the upgrade path):
// this is a SEQUENTIAL per-sequence recurrence (the exact same gated delta rule as the decode
// step, applied token-by-token), NOT the chunk-parallel WY-representation algorithm vLLM's
// FlashInfer/Triton/CuteDSL kernels use. It is correctness-first: numerically it matches the
// recurrence, but it does not exploit intra-sequence parallelism, so on long prefills it will NOT
// beat the chunked incumbents. It exists to (a) be the reference CUDA implementation that passes
// parity (bench/test_gdn_chunk_scan.py) and (b) be the base the chunk/WY parallelization
// (chunk_delta_h / solve_tril / chunk_o via CUTLASS/CuteDSL -- see docs/plans/gdn-parallel-scan.md
// and docs/improvements/gdn-scan-raw-mma.md) is built on IF Phase-0 profiling shows the scan
// dominant. Enable only where the on-box `make bench-kernel` A/B wins.
//
// Recurrence (per head, state S = [Dk, Dv] f32) -- MUST be reconciled with vLLM v0.25.0's
// fused_recurrent / chunk_gated_delta_rule on the box before trusting parity:
...
// the shim must convert it or bail -- documented in gdn_prefill_backend.py.
//
